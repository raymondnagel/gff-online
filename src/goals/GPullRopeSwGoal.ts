...
const VEL_X: number = -25; // How fast the player should move west while pulling the rope
const VEL_Y: number = 10; // How fast the player should move south while pulling the rope

/**
 * This goal is used by the Player to pull toward the SW;
 * it is the counterpart to GPullRopeSeGoal, which has the Prophet
 * pulling toward the SE. They are almost identical, except that the
 * prophet version doesn't play sound effects. (Since they both happen at
 * the same time, we don't want to play the sounds twice, so we just
 * let the player version handle that.)
 */
export class GPullRopeSwGoal extends GGoal {
    ...
    public start(): void {
        ...
        (this.char as GPlayerSprite).pullRopeSW();
        GFF.AdventureContent.getSound().playSound('rope_stretch');
        this.doWiggle();
    }
    ...
    public doStep(_time: number, _delta: number): void {
        ...
        if (isHeaving) {
            ...
            if (currentCycle < TOTAL_PULLS && !this.char.isDoing('pull')) {
                GFF.AdventureContent.getSound().playSound('rope_stretch', 1.0, 1.0 + (0.1 * currentCycle));
                this.doWiggle();
            }
            (this.char as GPlayerSprite).pullRopeSW();
            ...
            this.char.faceDirection(Dir9.NE, true);
